...
def lookup_cases_from_db(factors, item_name, material_name):
    """从数据库中按 (item_name, material_name, factor_name) 精确查找已导入的案例。

    通过环境变量 AUTO_PROMPT_API_URL 获取后端地址，调用 /api/cases/lookup 接口。
    返回 dict: {factor_name: extraction_rule}，未命中的要素不在其中。
    """
    ...
    for factor in factors:
        ...
        url = f"{api_url}/api/cases/lookup?{params}"
        try:
            req = urllib.request.Request(url, headers=headers)
            with urllib.request.urlopen(req, timeout=5) as resp:
                data = json.loads(resp.read().decode("utf-8"))
                if data.get("found") and data.get("case"):
                    rule = data["case"].get("extraction_rule", "")
                    if rule:
                        matched[fname] = rule
                        print(f"[提示词库] ✓ '{fname}' 命中库中已有提示词")
        except Exception:
            pass  # 静默失败，回退到 AI 生成
...
def main():
    ...
    try:
        ...
        for idx, factor in enumerate(factors, 1):
            if factor['name'] in db_matched:
                rule_text = db_matched[factor['name']]
